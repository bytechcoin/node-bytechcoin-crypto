...
void cn_bytech_lite_slow_hash_v0(const Nan::FunctionCallbackInfo <
                                 v8::Value > &info)
{
    std::string hash = std::string();
    std::string data = std::string();
    ...
    if (info.Length() == 1)
    {
        ...
        if (!data.empty())
        {
            ...
            try
            {
                Crypto::cn_bytech_lite_slow_hash_v0(rawData.data(),
                                                    rawData.size(),
                                                    c_hash);
            } catch(const std::exception & e) {
                return Nan::ThrowError(e.what());
...
void cn_bytech_lite_slow_hash_v1(const Nan::FunctionCallbackInfo <
                                 v8::Value > &info)
{
    std::string hash = std::string();
    std::string data = std::string();
    ...
    if (info.Length() == 1)
    {
        ...
        if (!data.empty())
        {
            ...
            try
            {
                Crypto::cn_bytech_lite_slow_hash_v1(rawData.data(),
                                                    rawData.size(),
                                                    c_hash);
            } catch(const std::exception & e) {
                return Nan::ThrowError(e.what());
...
void cn_bytech_lite_slow_hash_v2(const Nan::FunctionCallbackInfo <
                                 v8::Value > &info)
{
    std::string hash = std::string();
    std::string data = std::string();
    if (info.Length() == 1)
    {
        ...
        if (!data.empty())
        {
            ...
            try
            {
                Crypto::cn_bytech_lite_slow_hash_v2(rawData.data(),
                                                    rawData.size(),
                                                    c_hash);
            } catch(const std::exception & e) {
                return Nan::ThrowError(e.what());
...
void InitModule(v8::Local < v8::Object > exports)
{
    ...
    exports->Set(Nan::New("cn_bytech_lite_slow_hash_v0").ToLocalChecked(),
                 Nan::New < v8::FunctionTemplate >
                 (cn_bytech_lite_slow_hash_v0)->GetFunction());

    exports->Set(Nan::New("cn_bytech_lite_slow_hash_v1").ToLocalChecked(),
                 Nan::New < v8::FunctionTemplate >
                 (cn_bytech_lite_slow_hash_v1)->GetFunction());

    exports->Set(Nan::New("cn_bytech_lite_slow_hash_v2").ToLocalChecked(),
                 Nan::New < v8::FunctionTemplate >
                 (cn_bytech_lite_slow_hash_v2)->GetFunction());
...
NODE_MODULE(bytechcoincrypto, InitModule);
